# giang: just re-use code from variant 3, remember to check pamameters
...
EMBEDDINGS_DIRECTORY = '../finetuned_embeddings/variant_2_cnn'
BEST_MODEL_PATH = 'model/patch_variant_2_cnn_best_model.sav'
...
def predict_test_data(model, testing_generator, device, need_prob=False, need_feature_only=False):
    ...
    with torch.no_grad():
        model.eval()
        for ids, url, file_batch, label_batch in tqdm(testing_generator):
