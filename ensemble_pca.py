...
from model import EnsembleModel, EnsemblePCAModel
import torch
...
def predict_test_data(model, testing_generator, device, need_prob=False, need_features=False):
    ...
    with torch.no_grad():
        model.eval()
        for ids, url_batch, features, label_batch in tqdm(testing_generator):
...
def train(model, learning_rate, number_of_epochs, training_generator, test_java_generator, test_python_generator):
    ...
    for epoch in range(number_of_epochs):
        ...
        total_loss = 0
        current_batch = 0
        for ids, url_batch, features, label_batch in tqdm(training_generator):
            features = features.to(device)
            label_batch = label_batch.to(device)
...
def do_train(args, num_features):
    ...
    print("Feature length: {}".format(len(feature_data['train'][0])))

    pca = PCA(n_components=num_features)

    print("Fit-transform train data")
    ...
    training_set = EnsemblePcaDataset(train_ids, id_to_label, id_to_url, id_to_feature)
    test_java_set = EnsemblePcaDataset(test_java_ids, id_to_label, id_to_url, id_to_feature)
    test_python_set = EnsemblePcaDataset(test_python_ids, id_to_label, id_to_url, id_to_feature)
...
def infer_dataset(partition, feature_path):
    ...
    val_set = EnsemblePcaDataset(val_ids, id_to_label, id_to_url, id_to_feature)
